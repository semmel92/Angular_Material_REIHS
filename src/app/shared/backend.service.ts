...
export class BackendService {
  ...
  public getKindergardens(): Observable<Kindergarden[]> {
    ...
    return this.http.get<Kindergarden[]>('http://localhost:5000/kindergardens').pipe(tap(() => this.setLoading(false)), catchError(error => {
  ...
  public getChildren(page: number, filter?: { kindergardenId?: number }, sort?: {
    field: string,
    order: 'asc' | 'desc'
  }): Observable<ChildResponse[]> {
    this.setLoading(true);
    ...
    return this.http.get<ChildResponse[]>(`http://localhost:5000/childs?_expand=kindergarden&${queryParams}`).pipe(tap(() => this.setLoading(false)), catchError(error => {
  ...
  public addChildData(child: Child): Observable<any> {
    this.setLoading(true);
    return this.http.post('http://localhost:5000/childs', child).pipe(tap(() => {
  ...
  public deleteChildData(childId: string): Observable<any> {
    this.setLoading(true);
    return this.http.delete(`http://localhost:5000/childs/${childId}`).pipe(tap(() => {
  ...
  public getKindergartenDetails(id: string): Observable<Kindergarden> {
    this.setLoading(true);
    return this.http.get<Kindergarden>(`http://localhost:5000/kindergardens/${id}`).pipe(tap(() => this.setLoading(false)), catchError(error => {
